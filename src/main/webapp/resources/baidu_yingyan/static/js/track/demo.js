...
var Test_ak = 'BiW8BVDNAtEMIbFERc0fLqiUNP2t7wrL';
var ServiceId = '122826';
...
//    	alert('loadSelectedTrackHistory');
        var start_time = timeLineControl.start_time = Util.js_strto_time(startTime);
        var end_time = timeLineControl.end_time = Util.js_strto_time(endTime);
        var tmpSize = 0;
        // 在已选的track中 循环加载
...
//                console.log('track list:', curIndex)
                if (!selectedTracks.hasOwnProperty(entity_name)) {
                    me.selectTrack(entity_name, user_name, state, $(e.target).hasClass('check-box'), curIndex);
                } else {
            ...
            selTracksPanel.delegate('li', 'click', function (e) {
                ...
                me.selectTrack_2(entity_name, user_name, $(e.target).hasClass('check-box'));
                var track = selectedTracks_2[entity_name];
...
//                    }
//                }
//                urls.jsonp('http://api.map.baidu.com/trace/v2/entity/listcolumn', params, sucCbk);
...
//        	console.log("communityId="+communityId);
			...
			me._service = {};
			me._service.name = communityName;
			//会去数据库获得，当前用户能查看的员工集合
//			var names_arr = [];// 创建数组
//			names_arr.push(4312);
//			names_arr.push(4309);
            var params = {
                service_id: me._service_id || ServiceId,
                ak: me._ak || Test_ak,
//                entity_names:names_arr.toString(),//传多个entity_name
                community_id:communityId,//自定义检索字段，不传，获取所有
				active_time: Util.js_strto_time(startTime),
            }
            var sucCbk = function (res) {
                if (res.status == 0) {
                    me.actives = res.total;//显示总数n    n/m
...
//                entity_names:names_arr.toString(),//显示总数m    n/m
...
//                params.entity_names = keyWord;
                params.user_name = keyWord;//默认是按entity_names可搜索
            ...
            urls.jsonp('http://api.map.baidu.com/trace/v2/entity/list', params, function (res) {
                if (res.status === 0) {
                    ...
                    for (i in res.entities) {
                        ...
                        tracks.entity_name = tracks.entity_name;
                        tracks.user_name = tracks.user_name;
...
//                params.entity_names = keyWord_2;
                params.user_name = keyWord_2;//默认是按entity_names可搜索
            }
            var time = new Date().setHours(0, 0, 0) / 1000;
            var before = before || function () {
                $('.panel-mask').show();
        ...
        // 实时监控模式下选择track 响应事件
        selectTrack: function (entity_name, user_name, state, setView, curIndex) {
            ...
            if (!selectedTracks.hasOwnProperty(entity_name)) {
                ...
                var track = trackModule.createTrack(this._service_id || ServiceId, this._ak || Test_ak, entity_name, user_name, pageSize, curIndex);
        ...
        // 历史轨迹模式下选择track 响应事件
        selectTrack_2: function (entity_name, user_name, del) {
            ...
            if (!selectedTracks_2.hasOwnProperty(entity_name)) {
                ...
                checkBox.attr("checked", "checked");
                var track = trackModule.createTrack(this._service_id || ServiceId, this._ak || Test_ak, entity_name, user_name, 10, curIndex);
                track.setChecked(true);
...
define('track/track', ['track/urls', 'track/draw', 'track/canvasLayer', 'track/util'], function (urls, drawModule, CanvasLayer, Util) {
    ...
    var trackModule = {
        createTrack: function (trace_id, ak, entity_name, user_name, pageSize, pageIndex) {
            if (!pageSize) {
                pageSize = 14;
            }
            return new Track(trace_id, ak, entity_name, user_name, pageSize, pageIndex);
        },
